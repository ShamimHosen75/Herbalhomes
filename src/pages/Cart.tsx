    ...
    <PageLayout>
      <section className="bg-accent py-6">
        <div className="container mx-auto px-4">
          ...
          <h1 className="text-2xl font-bold text-foreground mt-3">{t("cart.title")} ({t("cart.items_count", { count: String(itemCount) })})</h1>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                return (
                  <div key={`${item.product.id}-${item.variantId}`} className="flex gap-4 bg-card rounded-2xl border border-border p-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {item.stock <= 5 && (
                        <p className="text-[10px] text-discount mt-1">{t("cart.only_left", { count: String(item.stock) })}</p>
                      )}
                    </div>
                  </div>
                );
            ...
            <div className="lg:col-span-1">
              <div className="bg-card rounded-2xl border border-border p-5 sticky top-24">
                <h3 className="font-bold text-foreground mb-4">{t("cart.order_summary")}</h3>
                <div className="space-y-2.5 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-foreground">{t("cart.delivery_at_checkout")}</span>
                  </div>
